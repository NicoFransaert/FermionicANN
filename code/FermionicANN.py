...
if __name__ == '__main__':
    ...
    parser.add_argument('-index', default=53, type=int)
    ...
    if args.machine == 'rnn':
        ...
        combos = [i for i in itertools.product(*list(grid.values()))] + [50, 1, 5e-3, 'O', 100000]
        num_units, num_layers, lr, lrschedule, numsamples = combos[args.index]
    ...
    if args.machine == 'rnn':
        ...
        run_FRNN(systemData=system, num_units = num_units, num_layers = num_layers, learningrate = lr, lrschedule=lrschedule, numsamples = numsamples, numsteps = 5000, save_dir=save_dir) # or this for argumetns from grid
